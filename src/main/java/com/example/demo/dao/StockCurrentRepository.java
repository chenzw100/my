...
public interface StockCurrentRepository extends JpaRepository<StockCurrent,Long> {
    ...
    @Query(value="SELECT * FROM ( SELECT code, name,sum(hot_seven) as hot_seven,sum(hot_value) as hot_value, COUNT(id) as total_count from stock_current WHERE day_format BETWEEN ?1 AND ?2  GROUP BY code) as temp WHERE temp.total_count>24 ORDER BY total_count DESC ", nativeQuery = true)
