...
public interface StockTradeValInfoJobRepository extends JpaRepository<StockTradeValInfoJob,Long> {
    ...
    @Query(value="SELECT * FROM mystock_trade_val_job s WHERE s.yn=1 and s.rank_type=1 and s.yesterday_close_price<10000 and s.trade_amount<800 and s.day_format=?1 ORDER BY s.yesterday_turnover LIMIT ?2", nativeQuery = true)
    public List<StockTradeValInfoJob> doMe(String day, Integer num);

    @Query(value="SELECT s.day_format 'dayFormat',count(s.day_format)'countNum',sum(s.one_open_income_rate)'oneOpenIncomeRate',sum(s.one_close_income_rate)'oneCloseIncomeRate',sum(s.one_next_open_income_rate)'oneNextOpenIncomeRate',sum(s.one_next_close_income_rate)'oneNextCloseIncomeRate', sum(s.two_open_income_rate)'twoOpenIncomeRate',sum(s.two_close_income_rate)'twoCloseIncomeRate' FROM mystock_trade_val_job s WHERE s.yn=1 and s.rank_type =?1 and s.one_open_rate < 960 and s.day_format BETWEEN ?2 and ?3 ", nativeQuery = true)
